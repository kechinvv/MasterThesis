...
Необходимость выполнения анализа указателей обусловлена тем, что общая последовательность нерепрезентативна и не содержит в себе реальных переходов и состояний библиотеки. Рассмотрим простой пример с #ref(<calls_example>, supplement: "листинга").
...
Извлекаемая трасса при игнорировании объектов будет выглядеть примерно как в #ref(<trace_wrong_example>, supplement: "листинге").
...
При восстановлении поведенческой модели окажется, что мы допускаем запись после того, как закрыли ZipOutputStream. Очевидно, такая модель не соответствует действительности. Применив анализ указателей мы рассчитываем получить две идентичные трассы как в #ref(<trace_right_example>, supplement: "листинге").
...
Общая схема динамического извлечения трасс представлена на #ref(<dynamic_scheme>, supplement: "рисунке").
