...
Для извлечения трасс будут применяться как статический, так и динамический анализ. В рамках данной работы предполагается, что с помощью динамического подхода в автоматическом режиме будет получено сильно меньше трасс, чем с помощью статического. Более того, уже имеющиеся или сгенерированные тесты не гарантируют целостность получаемых трасс ввиду возможного мокирования и искусственного создания объектов с помощью reflection. По этой причине для проведения экспериментов будут предложены различные режимы работы, позволяющие:
- Смешивать трассы, полученные разными методами
- Выбирать конкретный метод извлечения трасс
- Требовать частичное совпадение статических трасс с динамическими, осуществляя фильтрацию
...
Что касается динамического подхода, в рамках данной работы мы остановимся на сборе исключительно последовательностей вызовов, игнорируя значения аргументов и состояние программы. Сделано это с целью ограничить сложность работы, так как первоочередной целью является апробация идеи автоматизированного подхода к извлечению поведенческих моделей. Сбор информации должен осуществляться через инструментацию и запуск тестов. При отсутствии тестов в проекте, предлагается выполнить их генерацию с помощью инструмента Kex@kex. Kex статически формирует определенные пути исполнения программы, формируя ограничения на состояние программы в SMT-решателе, и на основе решения генерирует тесты, искусственно восстанавливая состояние программы для следования определенному пути исполнения. Таким образом мы можем избежать проблемы наличия необходимого внешнего окружения для запуска тестов.
