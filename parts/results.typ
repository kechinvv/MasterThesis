...
Что касается эталонной модели, к ней есть вопросы, так как она явно может быть обобщена лучше, а состояние S9 играет роль начального, хотя вызова конструктора для неё нет. В целом это допустимо, если библиотека содержит фабрики объектов. Тем не менее, складывается впечатление, что это обработанный вручную и автоматически полученный КА. 
