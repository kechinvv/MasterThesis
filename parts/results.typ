...
Стоит сказать, что проблему отсутствия эталонов можно решить созданием собственных, однако это трудоемкая задача, о чем сообщалось в исследованиях с проведенным тестированием. Задача решалась усилиями нескольких человек, а все модели, которые можно было переиспользовать из других исследований, использовались повторно@medv. Поэтому в рамках данной работы задача составления эталонов не стояла.
...
Модель StringBuilder самая простая из рассматриваемых. Восстановленный КА представлен на #ref(<own_strbuilder>, supplement: "рисунке"). Легко заметить, что модель не противоречит реальному использованию StringBuilder. Получены явно не все возможные вызовы библиотеки, что допустимо -- попались не очень интересные и несодержательные проекты. Более длительное применение инструмента обогатит данную модель новыми состояниями и переходами. Примечательно, что восстановленный КА воспроизвёл наиболее популярный сценарий использования StringBuilder без каких-либо неточностей.
...
Signature содержит гораздо больше вызовов, чем рассмотренные ранее классы. Для полученной модели, представленной на #ref(<own_signature>, supplement: "рисунке"), существует эталон, но он содержит лишь небольшую часть действительно возможных вызовов и переходов. Из модели видно, что хоть КА и является аппроксимацией сверху, он содержит понятные паттерны переходов и сохраняет определенную обобщённость.
