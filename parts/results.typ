...
В качестве источника проектов использовался GitHub, так как наиболее наглядными и понятными примерами являются классы стандартной библиотеки, отсутствующие в Maven Central. Однако библиотеки с Maven Central в результате экспериментов показали наиболее высокий процент успешного применения фаззинга, что повышало качество получаемых моделей.
...
Полученная модель представлена на #ref(<own_zip>, supplement: "рисунке"), а эталонная -- на #ref(<ground_zip>, supplement: "рисунке"). Важно отметить, что к данному эталону не возникает вопросов о его полноте и методе получения. Из рисунков видно, что обе модели покрывают все необходимые вызовы. Можно заметить, что полученная модель не противоречит эталону, но не содержит всех переходов и является неполной. Более длительный поиск проектов и трасс может улучшить ситуацию, но вряд ли даст эталонный результат.
...
Для полученной модели, представленной на #ref(<own_signature>, supplement: "рисунке"), существует эталон, но он содержит лишь небольшую часть действительно возможных вызовов и переходов. Тем не менее, из модели видно, что явно выделяются определенные повторяющиеся паттерны переходов между состояниями, а также присутствуют их дубликаты. Это является следствием работы алгоритма восстановления и не может быть решено без потери подробностей КА или использования другого алгоритма.
...
При сравнении полученной модели с #ref(<own_socket>, supplement: "рисунка") и эталона с #ref(<ground_socket>, supplement: "рисунка") видно, что полученный КА явно не содержит всех вызовов, а также имеет недостаточное обобщение. Также здесь видна погрешность статического подхода в том, что потеряно создание объекта и некоторые переходы происходят без вызова конструктора. Тем не менее, из полученного КА видны дальнейшие точки для ручного исправления. Например, объединение методов доступа к данным (get) в один переход.
